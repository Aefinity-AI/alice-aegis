...
# ---------------------------------------------------------------------
# Test 6: token minted for one tool presented to another tool's shim.
# ---------------------------------------------------------------------
section "6. cross-shim token -> SHIM REFUSE: bad mac (own shim still ALLOWs)"
if bash "$(dirname "$0")/06_cross_shim_token_confusion.sh"; then
  echo "RESULT: PASS"
  pass=$((pass+1))
else
  echo "RESULT: FAIL"
