#!/usr/bin/env bash
# Escape test 6: a capability token minted for ONE tool must be REFUSED at
# every OTHER tool's shim, even when the action bytes hash-match (e.g. a
# string that is simultaneously valid file content and a valid shell
# command). Tokens are bound to the issuing tool by folding a tool_tag into
# the capability token's HMAC input (see src/capability.rs); each shim
# always verifies against its OWN fixed tag (src/bin/tool_shim_*.rs).
#
# This test targets the tool-bound `cap_issue_for_test` / `capability::issue`
# / `capability::verify` / `check_and_consume` signatures (all take an
# explicit tool_tag) and does not compile against the earlier tag-less API.
#
