#!/usr/bin/env bash
# launch_m7_ternary.sh — guarded launcher for the FULL M7a TERNARY QAT pretraining (the run the twin baselines) run.
#
#   arm:     configs/m7_ternary.json (BitLinear ternary QAT, 14,171,392 params)
...
#   ckpt:    checkpoints/m7_ternary.pt, atomic write, resumable (--resume);
...
LOG="$TB/logs/m7_ternary_full_2026-07-21.log"
CKPT="$TB/checkpoints/m7_ternary.pt"
UNIT=m7-ternary
...
# Smoke test 2026-07-18 (logs/m7_ternary_smoke_2026-07-18.log) measured
...
systemd-run --user --unit="$UNIT" \
    ...
    -p Nice=10 \
    "$VENV_PY" "$TB/train.py" \
        --config "$TB/configs/m7_ternary.json" \
        --threads "$THREADS" \
