...
**The falsification challenge.** The author maintains a standing, public
challenge (`CHALLENGE.md`): find any machine on which a conforming build
produces a different digest or fails witness verification, and the finding
is recorded in the research ledger, credited by name, as a deliverable in
its own right, not a bug to be quietly fixed. No payment is offered or
implied. Reviewers are invited to try before publication.
