...
//!
//! Spec: `docs/CIS-1_SPEC_DRAFT_v0.1.md`. Motivation:
...
//! no `unsafe`. Where the draft is silent or its normative constants do not
//! exist yet, the choice made here is marked `SPEC GAP` and must be ratified
//! (or replaced) before v1.0 freezes the golden corpus.
...
///
/// SPEC GAP (the largest in this module): the draft specifies inverse sqrt
/// via a 64-entry exponent LUT seed + exactly k=3 Newton–Raphson iterations
/// in Q2.30 with normative constants — but the constants do not exist yet,
/// so that mechanism is unimplementable as written. This stand-in replaces
/// it with the exact floor `isqrt`, which is the simplest bit-reproducible
/// option AND has zero free parameters: nothing to freeze, nothing to get
/// wrong, and `isqrt` is a unique mathematical function any implementation
/// can optimize freely. Recommendation: ratify exact-isqrt as normative and
/// demote LUT+Newton to a conforming-optimization note. Either way the
/// RMSNORM-I goldens below change if the draft freezes Newton constants
/// instead. Also unratified here: the 2^15 intermediate scale, the i16 gain
/// type, and `max(t, 1)` standing in for the float path's epsilon (it only
/// guards the all-zero vector, whose outputs are zero regardless).
///
