...
// ---------------------------------------------------------------------------
// Explicit `full_blocks` coverage for the block-cadence internals of
// `tmv_i8_avx2`/`tmm_tile` (flush cadence, tail handling), independent of
// any particular pipelining scheme.
// ---------------------------------------------------------------------------

/// `full_blocks = n_bytes / BLOCK_BYTES` drives how many blocks the tmv/tmm
/// loops see before the tail. Exercise `full_blocks` in
/// `{0, 1, 2, 3, 5, 64, 65, 129, 130}` explicitly (small counts, counts that
/// straddle the FLUSH_BLOCKS=64 boundary, and counts one past a
...
/// Any tmv accumulation scheme that flushes on some cadence tied to
/// `FLUSH_BLOCKS=64` (whether that is every 64 blocks, or every
/// `2*FLUSH_BLOCKS=128` blocks split across two parity accumulators) must
/// stay bit-identical at shapes that straddle those cadence boundaries.
/// This drives dim_in across widths that land one block short of, exactly
/// at, and one block past one and two 128-block windows (127/129/255/257
/// blocks straddle a boundary with an odd total), on top of the existing
/// single-flush-cycle boundary coverage in `v3_flush_boundary_shapes_are_bit_identical`.
#[test]
fn two_flush_cycle_boundary_shapes_are_bit_identical() {
    let dim_out = 3usize;
    ...
    for &fb in &full_blocks_cases {
        ...
        assert_identical(
            ...
            dim_out,
            dim_in,
            &format!("two-flush-cycle full_blocks={fb}"),
        );
        ...
        for v in [127i8, -127] {
            ...
            assert_identical(
                ...
                dim_out,
                dim_in,
                &format!("two-flush-cycle extreme full_blocks={fb} v={v}"),
            );
        }
    }
}
