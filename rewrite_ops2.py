...
pattern1 = re.compile(r'#\[target_feature\(enable\s*=\s*"avx2",\s*enable\s*=\s*"fma"\)\]\s*\n\s*pub unsafe fn ternary_matvec_avx2.*?pub fn ternary_matvec[^\n]*\n.*?\n.*?\n\}', re.DOTALL)

pattern2 = re.compile(r'#\[target_feature\(enable\s*=\s*"avx2",\s*enable\s*=\s*"fma"\)\]\s*\n\s*pub unsafe fn ternary_matmul_avx2.*?pub fn ternary_matmul[^\n]*\n.*?\n.*?\n\}', re.DOTALL)
...
pub unsafe fn ternary_matvec_avx2(output: &mut [f32], input: &[f32], weights_packed: &[u8], dim_out: usize, dim_in: usize, scale: f32) {
    init_unpack_lut();
    ...
    while row + 3 < dim_out {
        ...
        while col_packed + 8 <= packed_dim_in {
            ...
            for i in 0..4 {
                ...
                let w_avx0 = core::arch::x86_64::_mm256_insertf128_ps(core::arch::x86_64::_mm256_castps128_ps256(core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b0_0 * 4))), core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b0_1 * 4)), 1);
                ...
                let w_avx1 = core::arch::x86_64::_mm256_insertf128_ps(core::arch::x86_64::_mm256_castps128_ps256(core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b1_0 * 4))), core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b1_1 * 4)), 1);
                ...
                let w_avx2 = core::arch::x86_64::_mm256_insertf128_ps(core::arch::x86_64::_mm256_castps128_ps256(core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b2_0 * 4))), core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b2_1 * 4)), 1);
                ...
                let w_avx3 = core::arch::x86_64::_mm256_insertf128_ps(core::arch::x86_64::_mm256_castps128_ps256(core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b3_0 * 4))), core::arch::x86_64::_mm_loadu_ps(lut_ptr.add(b3_1 * 4)), 1);
        ...
        core::arch::x86_64::_mm256_storeu_ps(s0.as_mut_ptr(), sum0); core::arch::x86_64::_mm256_storeu_ps(s1.as_mut_ptr(), sum1);
        core::arch::x86_64::_mm256_storeu_ps(s2.as_mut_ptr(), sum2); core::arch::x86_64::_mm256_storeu_ps(s3.as_mut_ptr(), sum3);
    ...
    while row < dim_out {
        ...
        let mut s0 = [0.0f32; 8]; core::arch::x86_64::_mm256_storeu_ps(s0.as_mut_ptr(), sum0);
...
pub fn ternary_matvec(output: &mut [f32], input: &[f32], weights_packed: &[u8], dim_out: usize, dim_in: usize, scale: f32) {
    if output.len() < dim_out || input.len() < dim_in || weights_packed.len() < (dim_out * dim_in) / 4 {
        return; 
    }
    unsafe { ternary_matvec_avx2(output, input, weights_packed, dim_out, dim_in, scale); }
}
"""

new_matmul = """
pub fn ternary_matmul(output: &mut [f32], input: &[f32], weights_packed: &[u8], batch: usize, dim_out: usize, dim_in: usize, scale: f32) {
    if output.len() < batch * dim_out || input.len() < batch * dim_in || weights_packed.len() < (dim_out * dim_in) / 4 {
        return; 
    }
    for b in 0..batch {
        let out_slice = &mut output[b * dim_out..(b + 1) * dim_out];
        let in_slice = &input[b * dim_in..(b + 1) * dim_in];
        unsafe { ternary_matvec_avx2(out_slice, in_slice, weights_packed, dim_out, dim_in, scale); }
    }
}
"""

text = pattern1.sub(new_code, text)
text = pattern2.sub(new_matmul, text)
