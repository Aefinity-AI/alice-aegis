...
### DejaVu Sans Mono (aegis-uefi GOP boot console font)

`aegis-uefi/src/font.rs` is a 1-bit-per-pixel bitmap rasterization of
`/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf` (ASCII 32..126, 16x32
cells), generated by `scripts/gen_font.py`. DejaVu fonts are derived from the
Bitstream Vera fonts; both the Bitstream Vera License and the DejaVu fonts
license permit embedding, redistribution, and modification (including
rasterization into a bitmap) in a larger work, with no royalty and no
requirement to rename the resulting derived artifact. See
